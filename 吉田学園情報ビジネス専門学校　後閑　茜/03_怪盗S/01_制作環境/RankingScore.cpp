...
// マクロ定義
//*****************************************************************************
#define	TEXTURE_RANKINGSCORE "data/TEXTURE/number000.png"	// 読み込むテクスチャファイル名
...
void InitRankingScore(void)
{
	...
	if (bRank == true)
	{
		g_nHighScore[0] = 400000;
		g_nHighScore[1] = 350000;
		g_nHighScore[2] = 300000;
		g_nHighScore[3] = 200000;
		g_nHighScore[4] = 100000;
...
void MakeVertexRankingScore(LPDIRECT3DDEVICE9 pDevice)
{
	...
	{//段
		...
		{//桁
			// 頂点情報を設定	
			pVtx[0].pos = D3DXVECTOR3(220.0f + (60 * nCnt), 180.0f + (110.0f * nCnt1), 0.0f);//一つ目の頂点の場所指定
			pVtx[1].pos = D3DXVECTOR3(280.0f + (60 * nCnt), 180.0f + (110.0f * nCnt1), 0.0f);//二つ目の頂点の場所指定（右回り）
			pVtx[2].pos = D3DXVECTOR3(220.0f + (60 * nCnt), 280.0f + (110.0f * nCnt1), 0.0f);//三つ目の頂点の場所指定（右回り）
			pVtx[3].pos = D3DXVECTOR3(280.0f + (60 * nCnt), 280.0f + (110.0f * nCnt1), 0.0f);//三つ目の頂点の場所指定（右回り）
