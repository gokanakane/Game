...
void MakeVertexScore(LPDIRECT3DDEVICE9 pDevice)
{
	...
	{		
		// 頂点情報を設定	
		pVtx[0].pos = D3DXVECTOR3(1000.0f +(40 * nCnt), 0.0f, 0.0f);//一つ目の頂点の場所指定
		pVtx[1].pos = D3DXVECTOR3(1050.0f +(40 * nCnt), 0.0f, 0.0f);//二つ目の頂点の場所指定（右回り）
		pVtx[2].pos = D3DXVECTOR3(1000.0f +(40 * nCnt), 50.0f, 0.0f);//三つ目の頂点の場所指定（右回り）
		pVtx[3].pos = D3DXVECTOR3(1050.0f +(40 * nCnt), 50.0f, 0.0f);//三つ目の頂点の場所指定（右回り）
